...
const INITIAL_JOBS: Job[] = [
  ...
  },
  {
    id: "3",
    jobRef: "OP-3884-Z",
    siteName: "Brentwood Hub",
    mainContractor: "Kier Group",
    postcode: "CM14 4BA",
    currentPours: 8,
    contractMaxPours: 8,
    status: "completed",
  ...
  },
  {
    id: "5",
    jobRef: "OP-9921-B",
    siteName: "Marina Development",
    mainContractor: "Morgan Sindall",
    postcode: "EH1 1YZ",
    currentPours: 12,
    contractMaxPours: 10,
    status: "in-progress",
...
export const PortalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // from this provider's own JSX (below), which the calling component's
  // handleReloadDemoData() call now just arms rather than performs directly.
  const [reloadDemoConfirmOpen, setReloadDemoConfirmOpen] = useState(false);

  const performReloadDemoData = async () => {
    setDataLoading(true);
    ...
    if (dw.error || dj.error || ds.error) {
      ...
      toast.error("Failed to reset demo data", {
        description: dw.error?.message || dj.error?.message || ds.error?.message,
      });
      setDataLoading(false);
      return;
    }
    ...
    if (iw.error || ij.error || is_.error) {
      ...
      toast.error("Failed to seed demo data", {
        description: iw.error?.message || ij.error?.message || is_.error?.message,
      });
      setDataLoading(false);
      return;
    }
    ...
    setDataLoading(false);
    toast.success("Demo dataset successfully seeded.");
  };
  ...
  return (
    ...
    >
      {children}
      <ConfirmDialog
        open={reloadDemoConfirmOpen}
        onOpenChange={setReloadDemoConfirmOpen}
        tone="destructive"
        title="Restore default demo dataset?"
        message="This will replace the workers, jobs and shifts stored in the backend."
        confirmLabel="Restore"
        onConfirm={performReloadDemoData}
      />
    </PortalContext.Provider>
  );
};
