...
serve(async (req) => {
  ...
  try {
    ...
    emailHtml += '      </div>';
    emailHtml += '      ';
    emailHtml += '      <p class="text-title" style="margin: 0 0 16px; color: #e5e7eb; font-size: 16px; font-weight: 700;">Dear ' + (clientName || 'Valued Client') + ',</p>';
    emailHtml += '      <p class="text-secondary" style="margin: 0 0 24px; color: #9ca3af;">Please find attached the formal concrete works quote estimate <strong>#' + quoteRef + '</strong> for your review.</p>';
    emailHtml += '      ';
    ...
    emailHtml += '          <td class="text-secondary" style="padding: 14px 16px; font-weight: bold; color: #9ca3af; text-transform: uppercase; font-size: 10px; letter-spacing: 0.1em;">Net Subtotal</td>';
    emailHtml += '          <td class="text-title" style="padding: 14px 16px; text-align: right; font-weight: 900; color: #e5e7eb;">£' + Number(netTotal || 0).toLocaleString(undefined, { minimumFractionDigits: 2 }) + '</td>';
    emailHtml += '        </tr>';
    emailHtml += '        <tr class="dark-bg" style="background-image: linear-gradient(#1a1b1f, #1a1b1f); background-color: #1a1b1f; border-bottom: 1px solid #2e2e33;">';
    emailHtml += '          <td class="text-secondary" style="padding: 14px 16px; font-weight: bold; color: #9ca3af; text-transform: uppercase; font-size: 10px; letter-spacing: 0.1em;">UK Standard VAT (20%)</td>';
    emailHtml += '          <td class="text-title" style="padding: 14px 16px; text-align: right; font-weight: 900; color: #e5e7eb;">£' + Number(vatAmount || 0).toLocaleString(undefined, { minimumFractionDigits: 2 }) + '</td>';
    emailHtml += '        </tr>';
    emailHtml += '        <tr class="header-bg" style="background-image: linear-gradient(#26262B, #26262B); background-color: #26262B;">';
    emailHtml += '          <td class="text-title" style="padding: 16px; font-weight: 900; color: #e5e7eb; text-transform: uppercase; font-size: 11px; letter-spacing: 0.15em;">Concrete Works Total</td>';
    emailHtml += '          <td class="text-title" style="padding: 16px; text-align: right; font-weight: 900; color: #e5e7eb; font-size: 16px;">£' + Number(grossTotal || 0).toLocaleString(undefined, { minimumFractionDigits: 2 }) + '</td>';
    emailHtml += '        </tr>';
    emailHtml += '      </table>';
    emailHtml += '      ';
    emailHtml += '      <p class="text-secondary" style="margin: 0 0 24px; color: #9ca3af;">The attached PDF contains the complete bill of quantities, structural scopes, standard terms, and banking details.</p>';
    emailHtml += '      ';
    emailHtml += '      <div style="border-top: 1px solid #2e2e33; padding-top: 24px; margin-top: 32px;">';
    emailHtml += '        <p class="text-title" style="margin: 0 0 4px; color: #e5e7eb; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em;">Opus Form Billing</p>';
