...
describe('validateWorkerForDeployment', () => {
  ...
  test('blocks Telehandler without active Telehandler operator ticket', () => {
    const cscsTicket: Ticket = { id: 't1', type: 'CSCS', expiryDate: futureStr, ticketNumber: '123' };
  ...
  test('allows Telehandler with valid tickets', () => {
    const cscsTicket: Ticket = { id: 't1', type: 'CSCS', expiryDate: futureStr, ticketNumber: '123' };
    const teleTicket: Ticket = { id: 't2', type: 'Telehandler', expiryDate: futureStr, ticketNumber: '456' };
...
describe('getTicketStatus', () => {
  ...
  test('returns EXPIRING_SOON for dates within 30 days', () => {
    const ticket: Ticket = { id: 't1', type: 'CSCS', expiryDate: soonFutureStr, ticketNumber: '123' };
    expect(getTicketStatus(ticket)).toBe('EXPIRING_SOON');
  });

  test('returns VALID for dates far in future', () => {
    const ticket: Ticket = { id: 't1', type: 'CSCS', expiryDate: farFutureStr, ticketNumber: '123' };
