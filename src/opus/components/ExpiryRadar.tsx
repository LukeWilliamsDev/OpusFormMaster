...
export const ExpiryRadar: React.FC<ExpiryRadarProps> = ({ expiringTickets, onSelectWorker }) => {
  ...
  const getTicketStyle = (ticket: TicketInfo) => {
    if (ticket.diffDays < 0) {
      ...
      // <= 14 days (Amber Theme)
      return {
        cardBg: 'bg-amber-950/30 border-amber-900/30 hover:border-amber-700/50 hover:bg-amber-950/40',
      ...
      return {
        cardBg: 'bg-yellow-950/15 border-yellow-900/25 hover:border-yellow-800/45 hover:bg-yellow-950/25',
      ...
      // Valid (Emerald Theme)
      return {
        cardBg: 'bg-emerald-950/20 border-emerald-900/30 hover:border-emerald-700/50 hover:bg-emerald-950/30',
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between border-b border-[#2a2a2a] pb-3">
        ...
        <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Compliance Control</span>
      </div>
      ...
      <div className="bg-[#1e1e1e] border border-[#2e2e2e] rounded-xl p-4 space-y-4 shadow-2xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-xs font-semibold text-gray-400 uppercase tracking-wider">
          ...
          <span className="text-[10px] text-[#888] font-mono font-bold">Total: {sortedTickets.length} flagged</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {sortedTickets.length === 0 ? (
            <div className="py-8 text-center text-xs text-gray-500 col-span-full flex flex-col items-center justify-center gap-2">
              ...
              <span className="font-black uppercase tracking-widest text-[9px]">All active personnel are fully compliant</span>
            </div>
          ) : (
                ...
                >
                  <div className="flex flex-col gap-0.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border shrink-0 ${style.badgeBg}`}>
                        {style.statusText}
                      </span>
                    </div>
