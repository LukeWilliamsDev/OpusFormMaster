...
serve(async (req) => {
  ...
  try {
    ...
    let resendApiKey = config['RESEND_API_KEY'] || Deno.env.get('RESEND_API_KEY')
    if (!resendApiKey) {
    ...
    emailHtml += '      <img src="https://fgpthpxmiroyebrzjdzo.supabase.co/functions/v1/send-quote-pdf" alt="OPUS FORM" width="180" style="display: inline-block; border: 0; outline: none; text-decoration: none;" />';
    emailHtml += '    </div>';
    emailHtml += '    ';
    emailHtml += '    <!-- Body -->';
    ...
    emailHtml += '      </div>';
    emailHtml += '      ';
    emailHtml += '      <p class="text-secondary" style="margin: 0 0 24px; color: #9ca3af; -webkit-text-fill-color: #9ca3af !important; font-size: 12px; text-align: center;" data-ogsc="color: #9ca3af;">This link is secure and passwordless. It will expire on: <strong>' + formattedExpiry + ' (UK time)</strong></p>';
    emailHtml += '      ';
    ...
    emailHtml += '        <p class="text-title" style="margin: 0 0 4px; color: #e5e7eb; -webkit-text-fill-color: #e5e7eb !important; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em;" data-ogsc="color: #e5e7eb;">Opus Form Support</p>';
    emailHtml += '        <a href="mailto:support@opusform.co.uk" style="color: #526E8C; -webkit-text-fill-color: #526E8C !important; text-decoration: none; font-size: 12px; font-weight: 700;" data-ogsc="color: #526E8C;">support@opusform.co.uk</a>';
    emailHtml += '      </div>';
    emailHtml += '    </div>';
    emailHtml += '  </div>';
    emailHtml += '</div>';
